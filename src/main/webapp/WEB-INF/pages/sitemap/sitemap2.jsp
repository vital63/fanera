    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-8">
                ...
                <br>  <label> Cnc milling machines </label>
                <c:forEach items="${listVmc}" var="vmc">
                    <a href="vmc${vmc.id}">${vmc.model} (id ${vmc.productid})  </a> <br>
                </c:forEach> 

                <br>  <label> Horizontal machining centers </label>
                <c:forEach items="${listHmc}" var="hmc">
                    <a href="hmc${hmc.id}">${hmc.model} (id ${hmc.productid})  </a> <br>
                </c:forEach> 

                <br>  <label> Lathe CNC </label>
                <c:forEach items="${listLathe}" var="lathe">
                    <a href="lathe${lathe.id}">${lathe.model} (id ${lathe.productId})  </a> <br>
                </c:forEach> 

                <br>  <label> Grinders </label>
                <c:forEach items="${listGrinder}" var="grinder">
                    <a href="grinder${grinder.id}">${grinder.model} (id ${grinder.id})  </a> <br>
                </c:forEach> 

                <br>   <label>  <a href="/video">Video</a> </label>
                 <c:forEach items="${listVideo}" var="video">
                  <a href="/video/${video.url}">№ ${video.data} - ${video.headline}</a> <br>
                 </c:forEach> 
                  
                  
                  
                  
<!--                         <br>  <label> <a href="vmc?lang=ru">Cnc milling machines - RU</a> </label>
                            <c:forEach items="${listVmc}" var="vmc">
                                <a href="vmc${vmc.id}?lang=ru">${vmc.model} (id ${vmc.productid})  - RU </a> <br>
                            </c:forEach> 
                            
                          <br>  <label> <a href="hmc?lang=ru">Horizontal machining centers - RU</a> </label>
                            <c:forEach items="${listHmc}" var="hmc">
                                <a href="hmc${hmc.id}?lang=ru">${hmc.model} (id ${hmc.productid})  - RU </a> <br>
                            </c:forEach> 
                            
                          <br>  <label> <a href="lathes?lang=ru">Lathe CNC - RU</a> </label>
                            <c:forEach items="${listLathe}" var="lathe">
                                <a href="lathe${lathe.id}?lang=ru">${lathe.model} (id ${lathe.productId})  - RU </a> <br>
                            </c:forEach>   
